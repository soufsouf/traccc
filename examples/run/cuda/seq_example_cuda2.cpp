    ...
    for (unsigned int event = common_opts.skip;
...

        {
            traccc::performance::timer wall_t("Wall time", elapsedTimes);
                ...
                traccc::int_buf channel0_buf(cellsVec.size, device_mr ? device_mr : &(cuda_host_mr));
                cellsView.channel0 = channel0_buf;
                traccc::int_buf channel1_buf(cellsVec.size,device_mr ? device_mr : &(cuda_host_mr));
                cellsView.channel1 = channel1_buf;
                traccc::scalar_buf activation_buf(cellsVec.size, device_mr ? device_mr : &(cuda_host_mr));
                cellsView.activation = activation_buf;
                traccc::scalar_buf time_buf(cellsVec.size, device_mr ? device_mr : &(cuda_host_mr));
                cellsView.time = time_buf;
                traccc::int_buf module_id_buf(cellsVec.size, device_mr ? device_mr : &(cuda_host_mr));
                cellsView.module_id = module_id_buf;
                traccc::int_buf cluster_id_buf(cellsVec.size, device_mr ? device_mr : &(cuda_host_mr));
                ...
                async_copy(vecmem::get_data(cellsVec.cluster_id), cluster_id_buf,
                    vecmem::copy::type::copy_type::host_to_device);
     /**************************************************/
                traccc::int_buf cells_prefix_sum(moduleVec.size, device_mr ? device_mr : &(cuda_host_mr));
